...
from functions import M_transform, f_seconds2sentence
...
# use regularization to bound the weights norm 
def f_regularized_loss(criterion,output, target, model, lambd):
    loss = criterion(output, target)
    # Regularization term
    for n in range(3):
        W = getattr(model, f'layer{n+1}').weight
        norm = f_tubal_nuclear_norm(W)
        loss += lambd*norm
    return loss   
...
def f_tubal_nuclear_norm(A):
    ...
    for i in range(A.shape[2]):
        U, S, V = torch.svd(A[:,:,i])
        t_nucelar_norm += torch.norm(S, p=1)
    return t_nucelar_norm
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            cost = f_regularized_loss(loss,outputs,labels,model,lamb_4_norm)

            optimizer.zero_grad()
            cost.backward()
            optimizer.step()
            total_loss_adv += loss(outputs, labels).item()
        ...
        if epoch > 0:
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'op-3L-{dim_latent}-regu-fnorms.png')
            plt.clf()
            # erank
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig(f'op-3L-{dim_latent}-regu-erank.png')
            plt.clf()
            # loss
            ...
            #plt.legend()
            plt.grid(True)
            plt.savefig(f'op-3L-{dim_latent}-regu-advloss.png')
            # do not save model
            torch.save(checkpoint_no_model, f'checkpoints/op-3L-{dim_latent}no-model-regu.pth')
        # Save the information after each epoch
        if (epoch+1)%100 == 0:
            torch.save(checkpoint, f'checkpoints/over-para-3Layer-epoch{epoch+1}-latentdim{dim_latent}-regu.pth')
